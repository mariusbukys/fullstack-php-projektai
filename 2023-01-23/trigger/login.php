...
if(!empty($_POST) AND isset($_GET['action']) AND $_GET['action'] === 'new_user') {
    ...
    if(!empty($_FILES['user-photo']['tmp_name'])){
      if(!is_dir('./userphoto')){
          mkdir('./userphoto');
      }
      $filename = explode('.',$_FILES['user-photo']['name']);
      $filename = time().'.'. $filename[count($filename)-1];
      $fileTypes = ['image/jpeg','image/png','image/gif'];
  
      if(!in_array($_FILES['user-photo']['type'],$fileTypes)){
          header('Location: ./messages.php?message=Incorrect file format&status=danger');
          exit;
      }
      move_uploaded_file($_FILES['user-photo']['tmp_name'], './userphoto/' . $filename);
      $data['person'][count($data['person'])-1]['user-photo'] = $filename;
    }
...
<div class="mt-5">
    <h1 class="fw-bold text-info-emphasis fst-italic text-decoration-underline display-1"><i class="bi bi-binoculars-fill"></i> TWIGGER</h1>
...
<?php if(isset($_GET['action']) AND $_GET['action'] === 'login') : ?>
    ...
    <main class="form-signin w-25 m-auto">
      <form method="POST" enctype="multipart/form-data">
        ...
        <label for="photo" class="border border-dark px-3 rounded mb-1"><i class="bi me-2 fs-2 bi-person-circle"></i> Vartotojo nuotrauka</label>
